...
- `node_type`：节点类型
- `title`：节点标题
- `permission_grant`（可选）：CLI 自动为当前用户补授权限时返回，说明是否已自动为当前 CLI 用户授予可管理权限

> [!IMPORTANT]
> 节点创建成功后，CLI 会**尝试为当前 CLI 用户自动授予该知识库节点的 `full_access`（可管理权限）**。
>
...
- **自动授权**：创建成功后，结果还会额外带上 `permission_grant`，用于说明是否已自动为当前 CLI 用户授予新建节点的可管理权限
