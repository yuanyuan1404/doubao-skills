...
- Base 文档只保留会影响 Base 路径选择的权限规则；遇到 scope/授权不足，根据错误响应中的 `console_url` 引导用户解决。
...
- 统一以 user 身份操作用户资源。
- user 身份报 scope/授权不足，或错误中包含 `permission_violations` / `hint`，根据错误响应中的 `console_url` 引导用户解决，不要循环重试。
- `91403` 或明确不可访问错误不要循环重试。
- `+base-create` / `+base-copy` 返回中若带 `permission_grant`，关注该字段并把用户是否可打开新 Base 告知用户。
...
| `91403` | 无权限访问该 Base，根据错误响应中的 `console_url` 引导用户解决，不要盲目重试 |
